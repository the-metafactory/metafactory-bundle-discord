...
The same bot can post to any guild it has joined (e.g. two different servers).
Pick the guild for channel/thread name resolution with one of:

```bash
# Saved profile (preferred — register once with `discord config set-server`):
discord post --server myserver "Deployed v0.6.0 to staging"
