...
// Example guild IDs: a top-level guild + a second one used as a named profile.
const GROVE_GUILD = "100000000000000001";
const HALDEN_GUILD = "200000000000000002";
  ...
  return {
    ...
    servers: {
      halden: {
        ...
        channels: { general: { id: "200000000000000003" } },
      },
    },
  };
}
...
describe("resolveServerContext — --server profile (ISC-C7/C8)", () => {
  ...
  test("--server uses the profile's defaultChannel and channels", () => {
    ...
    expect(ctx.channels).toEqual({ general: { id: "200000000000000003" } });
...
describe("cachedChannelId — guild-scoped channel cache (cortex#1030)", () => {
  ...
  test("--server can use its own profile channel cache", () => {
    const ctx = resolveServerContext(baseConfig(), { server: "halden" });
    expect(cachedChannelId(ctx, "general")).toBe("200000000000000003");
  });
